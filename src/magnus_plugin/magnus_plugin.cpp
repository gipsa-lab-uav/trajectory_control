...
/// \brief: \TODO: Update to use as magnus plugin (was copied from liftdrag_plugin.cpp)
GZ_REGISTER_MODEL_PLUGIN(MagnusPlugin)

/////////////////////////////////////////////////
MagnusPlugin::MagnusPlugin() : cla(1.0), cda(0.01), cma(0.01), rho(1.2041)
{
...
/////////////////////////////////////////////////
void MagnusPlugin::Load(physics::ModelPtr _model,
                     sdf::ElementPtr _sdf)
{
  GZ_ASSERT(_model, "MagnusPlugin _model pointer is NULL");
  GZ_ASSERT(_sdf, "MagnusPlugin _sdf pointer is NULL");
  this->model = _model;
  this->sdf = _sdf;

  this->world = this->model->GetWorld();
  GZ_ASSERT(this->world, "MagnusPlugin world pointer is NULL");
...
#endif
  GZ_ASSERT(this->physics, "MagnusPlugin physics pointer is NULL");
  ...
  if (_sdf->HasElement("link_name"))
  {
    ...
    if (!this->link)
    {
      gzerr << "Link with name[" << linkName << "] not found. "
        << "The MagnusPlugin will not generate forces\n";
    }
    else
    {
      this->updateConnection = event::Events::ConnectWorldUpdateBegin(
          boost::bind(&MagnusPlugin::OnUpdate, this));
    }
  }
