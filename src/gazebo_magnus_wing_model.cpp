...
/// \brief: \TODO: Update to use as magnus model (was copied from gazebo_motor_model.cpp)
// Use wind and drone velocities to calculate apparent wind, use the magnitude
// in XZ-plane to calculate spin ratio. Lift is orthogonal to the velocity vector,
// and drag is parallel. Spin ratio is upper bounded at 8. Coefficients for drag in
// XZ-plane, Y-axis and lift are CD, CDy and CL. X1 is spin ratio:
// CD  = -0.0211*X1.^3 + 0.1873*X1.^2 + 0.1183*X1    + 0.5
// CL  =  0.0126*X1.^4 - 0.2004*X1.^3 + 0.7482*X1.^2 + 1.3447*X1
// CDy =  0.8
// Lift force is 0.5*rhoAir*mSurface*CL*(norm(WappVel)^2)
// Drag on XZ is 0.5*rhoAir*mSurface*CD*(norm(WappVel)^2)
// Drag on Y is 0.5*rhoAir*mLatSurface*CDy*(norm(WappVel)^2)
// Gyroscopic torques are Iy*mAngSpeed*Omega(yaw) in X,
// Iy*mAngSpeed*Omega(roll) in Z and zero in Y.
// Iy is (mMass*mRadius^2)/2.

namespace gazebo {

GazeboWingModel::~GazeboWingModel() {
  updateConnection_->~Connection();
  use_pid_ = false;
}

void GazeboWingModel::InitializeParams() {}

void GazeboWingModel::Publish() {
  turning_velocity_msg_.set_data(joint_->GetVelocity(0));
  motor_velocity_pub_->Publish(turning_velocity_msg_);
}

void GazeboWingModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  model_ = _model;
  ...
  if (_sdf->HasElement("wingNumber"))
    motor_number_ = _sdf->GetElement("wingNumber")->Get<int>();
  else
  ...
  if (_sdf->HasElement("turningDirection")) {
    ...
    if (turning_direction == "cw")
      ...
      gzerr << "[gazebo_magnus_wing_model] Please only use 'cw' or 'ccw' as turningDirection.\n";
  }
  else
    gzerr << "[gazebo_magnus_wing_model] Please specify a turning direction ('cw' or 'ccw').\n";
  ...
  motor_sub_ = node_handle_->Subscribe<mav_msgs::msgs::MotorSpeed>("~/" + model_->GetName() + motor_test_sub_topic_, &GazeboWingModel::testProto, this);
  */

  // Listen to the update event. This event is broadcast every
  // simulation iteration.
  updateConnection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&GazeboWingModel::OnUpdate, this, _1));

  command_sub_ = node_handle_->Subscribe<mav_msgs::msgs::CommandMotorSpeed>("~/" + model_->GetName() + command_sub_topic_, &GazeboWingModel::VelocityCallback, this);
  //std::cout << "[gazebo_magnus_wing_model]: Subscribe to gz topic: "<< motor_failure_sub_topic_ << std::endl;
  motor_failure_sub_ = node_handle_->Subscribe<msgs::Int>(motor_failure_sub_topic_, &GazeboWingModel::MotorFailureCallback, this);
...
void GazeboWingModel::OnUpdate(const common::UpdateInfo& _info) {
...
void GazeboWingModel::VelocityCallback(CommandMotorSpeedPtr &rot_velocities) {
...
void GazeboWingModel::MotorFailureCallback(const boost::shared_ptr<const msgs::Int> &fail_msg) {
...
void GazeboWingModel::UpdateMotorFail() {
...
GZ_REGISTER_MODEL_PLUGIN(GazeboWingModel);
}
